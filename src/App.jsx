...
function App() {
  ...
  useEffect(() => {
    if (!isAuthenticated || !accessToken || restaurantName?.trim()) return;

    let cancelled = false;

    (async () => {
      const resolvedName = await resolveRestaurantName(accessToken, restaurantId);
      if (!cancelled && resolvedName) {
        dispatch(setRestaurantName(resolvedName));
      }
    })();
  ...
  }, [accessToken, dispatch, isAuthenticated, restaurantId, restaurantName]);
